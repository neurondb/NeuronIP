...
/* AzureSynapseConnector implements the Connector interface for Azure Synapse Analytics */
type AzureSynapseConnector struct {
	*ingestion.BaseConnector
	db *sql.DB
}

/* NewAzureSynapseConnector creates a new Azure Synapse connector */
func NewAzureSynapseConnector() *AzureSynapseConnector {
	metadata := ingestion.ConnectorMetadata{
		Type:        "azuresynapse",
		Name:        "Azure Synapse Analytics",
		Description: "Azure Synapse Analytics connector for schema discovery and data sync",
		Version:     "1.0.0",
		Capabilities: []string{"incremental", "schema_discovery", "full_sync", "query_log_analysis"},
	}

	base := ingestion.NewBaseConnector("azuresynapse", metadata)

	return &AzureSynapseConnector{
		BaseConnector: base,
	}
}

/* Connect establishes connection to Azure Synapse */
func (a *AzureSynapseConnector) Connect(ctx context.Context, config map[string]interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to open Azure Synapse connection: %w", err)
	}

	if err := db.PingContext(ctx); err != nil {
		return fmt.Errorf("failed to ping Azure Synapse: %w", err)
	}
...
func (a *AzureSynapseConnector) Disconnect(ctx context.Context) error {
	if a.db != nil {
		a.db.Close()
	}
...
func (a *AzureSynapseConnector) TestConnection(ctx context.Context) error {
	if a.db == nil {
...
/* DiscoverSchema discovers Azure Synapse schema */
func (a *AzureSynapseConnector) DiscoverSchema(ctx context.Context) (*ingestion.Schema, error) {
	if a.db == nil {
		return nil, fmt.Errorf("not connected")
	}

	// Azure Synapse uses similar schema as SQL Server
	query := `
...
func (a *AzureSynapseConnector) getColumns(ctx context.Context, schemaName, tableName string) ([]ingestion.ColumnSchema, error) {
	query := `
...
func (a *AzureSynapseConnector) Sync(ctx context.Context, options ingestion.SyncOptions) (*ingestion.SyncResult, error) {
	if a.db == nil {
		return nil, fmt.Errorf("not connected")
	}
